function [FO, FD, cleanUpObj] = jmim_rw(features, targets, varargin)
    ...
    [FO, FD, cleanUpObj] = feasel_frame(@jmim_rw_algorithm, features, targets, it_struct, varargin{:});

end

function [FO, FD] = jmim_rw_algorithm(features, target, varargin)
    ...
    % load([fullPath_firstPart pvStruct.target_mat '.mat'], 'MIit', 'JMIijt');
    %load(pvStruct.target_mat, 'MIit', 'JMIijt');
    ...
    while ~isempty(calcList) && stop_flag ~= 1
        ...
        ss = zeros(1, n);
        for num = 1 : n
            jmi = getSetJMI(aktList(:,1:mes_ind), calcList(:, num), m);
            ss(1, num) = jmi;
        end
        mes_ind = mes_ind + 1;
...
function minJMI = getSetJMI(set, Fi, m)
    sSet = size(set, 2);
    JMIpqc = zeros(1, sSet);
    Fi_ind = Fi(2,1);
    for idx = 1 : sSet
        Fp_ind = set(2, idx);
        if Fp_ind < Fi_ind, c = m.getJMIijt(Fp_ind, Fi_ind); else c = m.getJMIijt(Fi_ind, Fp_ind); end % upper triangular
        JMIpqc(1, idx) = c;
    end
